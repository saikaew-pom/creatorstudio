...
import { listCompanies } from "@cs/db";
...
export default async function CompaniesPage() {
  ...
  if (gate.kind === "signed-out") {
    return (
      <div className="empty-state">
        ...
        <p>เข้าสู่ระบบก่อนเพื่อดูบริษัท</p>
        <Link href="/login?next=/crm/companies" className="btn primary" style={{ marginTop: 10 }}>เข้าสู่ระบบ</Link>
      </div>
    );
  }
  ...
  const companies = await listCompanies(getServerSupabase(), wsId);

  return <CompaniesPanel initialCompanies={companies} wsId={wsId} />;
}
